...
int main()
{
	...
	while (cond) try {
		...
		case NUMBER: {
			Document doc;
			istringstream iss;
			iss.str("Title\nThe1st line of the document\nAnother.line\nUPPER_CASE_LETTERS\nLast line...");
			iss >> doc;
			cout << endl << sp_2 << "Initial Document:" << endl;
			print(doc);

			cout << endl << endl << sp_2 << "Number of words (a whitespace-separated sequence of characters):"
				<< endl << sp_4 << doc.NumberOfWords(WordType::WHITESPACESEPARATED);
			cout << endl << sp_2 << "Number of words (a sequence of consecutive alphabetic characters):"
				<< endl << sp_4 << doc.NumberOfWords(WordType::CONSECUTIVEALPHABETIC);
...
long long int Document::NumberOfCharacters() const
{
	long long int number = 0;
	list<Line>::const_iterator it = line.begin();
	while (it != line.end()) {
		number += (it->end() - it->begin());
		++it;
	}
	return number;
}

long long int Document::NumberOfWords(WordType type)
{
	long long int number = 0;
	bool isWord = false;
	Text_iterator it = begin();
	if (type == WordType::WHITESPACESEPARATED) {
		while (it != end()) {
			if (isspace(*it)) {
				if (isWord) {
					number++;
	...
	else if (type == WordType::CONSECUTIVEALPHABETIC) {
		while (it != end()) {
			if (isalpha(*it)) isWord = true;
			else {
				if (isWord) {
					number++;
					isWord = false;
