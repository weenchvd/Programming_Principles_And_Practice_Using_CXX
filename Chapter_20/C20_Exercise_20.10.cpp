...
int main()
{
	...
	while (cond) try {
		...
		case NUMBER: {
			...
			vector<char> spaces = { '\t', ' ' };
			cout << endl << endl << sp_2 << "Number of words (a set of whitespace characters):"
				<< endl << sp_4 << doc.NumberOfWords(spaces);
...
long long int Document::NumberOfWords(const vector<char>& spaces)
{
	long long int number = 0;
	bool isWord = false;
	Text_iterator it = begin();
	while (it != end()) {
		if (isIncluded(*it, spaces)) {
			if (isWord) {
				number++;
				isWord = false;
			}
...
bool Document::isIncluded(char ch, const vector<char>& spaces)
{
	for (int i = 0; i < spaces.size(); i++) {
		if (ch == spaces[i]) return true;
	}
	return false;
}
