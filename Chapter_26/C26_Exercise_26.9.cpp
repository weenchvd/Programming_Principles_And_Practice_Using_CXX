...
My_window::My_window(Point xy, int w, int h, const string& title)
	...
	outboxOutput{ Point{ 200, 20 }, 600, 20, "Output:" },
...
void My_window::DrawShape()
{
	...
	if (shape != nullptr) {
		...
		attach(*shape);
		ostringstream oss;
		TextInterface::PrintShapeCommand(oss, shape);
		outboxOutput.put(oss.str());
	}
	else {
		outboxOutput.put("");
	}
...
TextInterface::Token TextInterface::TokenStream::GetToken()
{
	...
	default: {
		if (isalpha(ch)) {
			...
			if (!(*ios.in)) return Token{ TokenType::INVALID };
			ios.in->unget();
			if (s == Command::line) return Token{ TokenType::LINE };
			if (s == Command::rectangle) return Token{ TokenType::RECTANGLE };
			if (s == Command::circle) return Token{ TokenType::CIRCLE };
			if (s == Command::clpolyline) return Token{ TokenType::CLOSEDPOLYLINE };
			if (s == Command::point) return Token{ TokenType::POINT };
...
void TextInterface::PrintShapeCommand(ostream& os, Shape* sh)
{
	if (typeid(*sh) == typeid(Graph_lib::Line)) {
		Graph_lib::Line* p = static_cast<Line*>(sh);
		os << Command::line << '(';
		os << Command::point << '(' << p->point(0).x << ',' << p->point(0).y << ')' << ',';
		os << Command::point << '(' << p->point(1).x << ',' << p->point(1).y << ')';
		os << ')';
	}
	else if (typeid(*sh) == typeid(Graph_lib::RectanglePPP2)) {
		Graph_lib::RectanglePPP2* p = static_cast<RectanglePPP2*>(sh);
		os << Command::rectangle << '(';
		os << Command::point << '(' << p->point(0).x << ',' << p->point(0).y << ')' << ',';
		os << p->width() << ',' << p->height();
		os << ')';
	}
	else if (typeid(*sh) == typeid(Graph_lib::Circle)) {
		Graph_lib::Circle* p = static_cast<Circle*>(sh);
		os << Command::circle << '(';
		os << Command::point << '(' << p->point(0).x << ',' << p->point(0).y << ')' << ',';
		os << p->radius();
		os << ')';
	}
	else if (typeid(*sh) == typeid(Graph_lib::Closed_polyline)) {
		Graph_lib::Closed_polyline* p = static_cast<Closed_polyline*>(sh);
		os << Command::clpolyline << '(';
		for (int i = 0; i < p->number_of_points(); ++i) {
			os << Command::point << '(' << p->point(i).x << ',' << p->point(i).y << ')';
			if (i + 1 < p->number_of_points()) os << ',';
		}
		os << ')';
	}
