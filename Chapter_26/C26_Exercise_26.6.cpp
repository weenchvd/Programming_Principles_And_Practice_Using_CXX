...
int TestCalculator(const string& inputFile, const string& outputFile)
{
	IOFStream io;
	io.ifs.open(inputFile);
	if (!io.ifs) {
		io.ifs.clear();
		io.ifs.close();
		cout << "Error. Can't open input file: " + inputFile;
		return -1;
	}
	io.ofs.open(outputFile);
	if (!io.ofs) {
		io.ofs.clear();
		io.ofs.close();
		cout << "Error. Can't open output file: " + outputFile;
		return -1;
	}
	return Calculator(io);
}

bool CheckInput(ifstream& ifs1, ifstream& ifs2, int line)
{
	if (ifs1 && ifs2) return true;
	cout << sp_4 << "Read error: line " << line << endl;
	string errNumber{ "The number of tests does not match: " };
	if (ifs1) {
		if (ifs2.eof()) cout << sp_6 << errNumber << "Solution file" << endl;
		else cout << sp_6 << "Solution file error" << endl;
	}
	else if (ifs2) {
		if (ifs1.eof()) cout << sp_6 << errNumber << "Input file" << endl;
		else cout << sp_6 << "Input file error" << endl;
	}
	else {
		cout << sp_6 << "An error occurred with both files" << endl;
	}
	return false;
}
...
{
	try {
		string inputFile{ "C26_Exercise_26.6_input.txt" };
		string outputFile{ "C26_Exercise_26.6_output.txt" };
		TestCalculator(inputFile, outputFile);
		ifstream ifsOutput{ outputFile };
		if (!ifsOutput) error("Error. Can't open input file: ", outputFile);
		string solutionFile{ "C26_Exercise_26.6_solution.txt" };
		ifstream ifsSolution{ solutionFile };
		if (!ifsSolution) error("Error. Can't open input file: ", solutionFile);
		...
		while (true) {
			...
			ifsOutput >> ch >> out;
			ifsSolution >> sol;
			if (ifsOutput.eof() && ifsSolution.eof()) break;
			if (!CheckInput(ifsOutput, ifsSolution, lineCount)) return -1;
			if (ch != '=') cout << sp_2 << "Invalid prefix" << endl;
			if (out != sol) {
				cout << sp_2 << "Test failed: line " << lineCount << " (" << out << " != " << sol << ")" << endl;
				++errorCount;
			}
		}
		if (errorCount) cout << sp_8 << errorCount << " error(s)" << endl;
		else cout << endl << sp_8 << "All tests passed successfully" << endl;
