...
try {
	...
	sw.attach(bt1);
	bt1.SetLabel("", "AB");
	bt1.SetLabel("llr", "DDDDDD");
	bt1.SetLabel("rrr", "xFF");
	bt1.SetLabel("lr", "U");
	...
	btt1.SetLabel("l", "L2");
	btt1.SetLabel("r", "R2");
	btt1.SetLabel("lll", "L4");

	Binary_tree_rhombus btr1{ Point{600, 600}, 5, Link_type::lineArrowUp, Color::dark_yellow };
	sw.attach(btr1);
	btr1.SetLabel("", "RT");
	btr1.SetLabel("rll", "X21");
	btr1.SetLabel("rlr", "X222");
	btr1.SetLabel("rrrr", "Y");
...
void Binary_tree::DrawLabel(Point rootCenter, const NodeLabel& lab) const
{
	constexpr int yShift = radius / 4;
	const int depth = lab.loc.size();
	if (depth >= lvl) error("location is invalid");
	int currentLevel = 1;
	for (int i = 0; i < depth; i++) {
		const int dif = lvl - i - 1;
		int xIndentation = dist / 2;
		for (int i = 1; i < dif; i++) {
			xIndentation *= 2;
		}
		if (lab.loc[i] == 'l') rootCenter.x -= xIndentation;
		else rootCenter.x += xIndentation;
		rootCenter.y += dist;
	}
	const int n = (lab.text.size() < nChars) ? lab.text.size() : nChars;
	Point text = Point{ rootCenter.x - int(fl_width(lab.text.c_str(), n) / 2), rootCenter.y + yShift };
	int ofnt = fl_font();
	int osz = fl_size();
	fl_font(fnt.as_int(), fnt_sz);
	fl_draw(lab.text.c_str(), n, text.x, text.y);
	fl_font(ofnt, osz);
}
...
void Binary_tree_triangle::draw_lines() const
{
	...
	if (GetLevel() > 0) {
		...
		for (int i = 0; i < lab.size(); i++) {
			Binary_tree::DrawLabel(center, lab[i]);
		}
		center = point(0);
		DrawChildren(center, 1);
	}
...
void Binary_tree_rhombus::draw_lines() const
{
	if (GetLevel() > 0) {
		Point center = point(0);
		Point p1 = GetPointOnCircle(center, GetRadius(), 0);
		Point p2 = GetPointOnCircle(center, GetRadius(), 90);
		Point p3 = GetPointOnCircle(center, GetRadius(), 180);
		Point p4 = GetPointOnCircle(center, GetRadius(), 270);
		fl_line(p1.x, p1.y, p2.x, p2.y);
		fl_line(p2.x, p2.y, p3.x, p3.y);
