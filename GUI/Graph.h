...
struct RectanglePPP2 : Shape {

    RectanglePPP2(Point xy, int ww, int hh) : w(ww), h(hh)
    {
        add(xy);
        if (h<=0 || w<=0) error("Bad rectangle: non-positive side");
    }

    RectanglePPP2(Point x, Point y) : w(y.x-x.x), h(y.y-x.y)
    {
        add(x);
...
struct PolygonPPP2 : Closed_polyline {    // closed sequence of non-intersecting lines
    void add(Point p);
    void draw_lines() const;
};
...
struct EllipsePPP2 : Shape {
    EllipsePPP2(Point p, int w, int h)    // center, min, and max distance from center
        : w(w), h(h)
    { 
