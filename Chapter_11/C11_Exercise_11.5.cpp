/* Exercise 11.5 */

#include<iostream>
#include<string>
#include<vector>
#include<algorithm>
#include<cmath>
#include<fstream>
#include<sstream>
#include<iomanip>
#include"C11_Exercise_11.5.h"

using namespace std;

inline void error(string s) { throw runtime_error(s); }
inline void error(const string& s, const string& s2) { error(s + s2); }
inline void error(const string& s, int i) { ostringstream os; os << s << ": " << i; error(os.str()); }
inline void keep_window_open() { char ch; cin >> ch; }

const string errMessageInputFile = "Error. Can't open input file: ";
const string errMessageOutputFile = "Error. Can't open output file: ";

int main()
{
	enum Action {
		EXIT = -1, PRINTACTIONLIST,
		CLASSIFYCHARACTERS
	};
	const string actionList = "\tList of actions:\n"
	"  (1) ClassifyCharacters()\n"
	"  (-1) Exit, (0) Print the list of actions\n";
	cout << actionList;
	int action;
	bool cond{ true };
	while (cond) try {
		cout << "\nPlease enter the action: ";
		if (!(cin >> action)) { ClearInput(cin); error("Error. Incorrect input"); }
		switch (action) {
		case CLASSIFYCHARACTERS: {
			string s;
			getline(cin, s);
			if (cin) ClassifyCharacters(s);
			else error("Bad input");
			break;
		}
		case PRINTACTIONLIST:
			cout << actionList;
			break;
		case EXIT:
			cond = false;
			break;
		default:
			error("Error. Incorrect action number");
			break;
		}
	}
	catch (runtime_error& e) {
		cerr << e.what() << endl;
	}
	catch (...) {
		cerr << "Error. Exception\n";
		return 1;
	}
	return 0;
}

void ClassifyCharacters(const string& s)
{
	for (char ch; cin.get(ch); ) {
		bool comma = false;
		cout << ch << ":";
		if (isspace(ch)) {
			cout << " whitespase";
			comma = true;
		}
		if (isalpha(ch)) {
			if (comma) cout << ",";
			if (isupper(ch)) cout << " uppercase letter";
			else cout << " lowercase letter";
			comma = true;
		}
		if (isdigit(ch)) {
			if (comma) cout << ",";
			cout << " decimal digit";
			comma = true;
		}
		if (isxdigit(ch)) {
			if (comma) cout << ",";
			if (isupper(ch)) cout << " uppercase hexadecimal digit";
			else if (islower(ch)) cout << " lowercase hexadecimal digit";
			else cout << " hexadecimal digit";
			comma = true;
		}
		if (iscntrl(ch)) {
			if (comma) cout << ",";
			cout << " control character";
			comma = true;
		}
		if (ispunct(ch)) {
			if (comma) cout << ",";
			cout << " punctuation character";
			comma = true;
		}
		if (isprint(ch)) {
			if (comma) cout << ",";
			cout << " printable";
			comma = true;
		}
		cout << endl;
		if (ch == '\n') return;
	}
}

void ClearInput(istream& is)
{
	is.clear();
	is.ignore(numeric_limits<streamsize>::max(), '\n');
}
