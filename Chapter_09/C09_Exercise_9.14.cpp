...
{
	enum Action {
		EXIT = -1, PRINTACTIONLIST, ADDITION, SUBTRACTION, MULTIPLICATION, DIVISION
	};
...
namespace Finance {
	Money::Money(long int dol, long int cent)
		: cents{ dol * 100 }
	{
	...
	istream& operator>>(istream& is, Money& m)
	{
		char ch1, ch2;
		long int dol, cent;
		if (!(is >> ch1 >> dol >> ch2 >> cent)) { ClearInput(is); error("Incorrect input"); }
		if (ch1 != '$' || ch2 != '.') { ClearInput(is); error("Incorrect input"); }
		m = Money(dol, cent);
		return is;
	}
...
{
	Finance::Money m;
	cout << "Please enter the amount of money in the form \"$xxxx.xx\": ";
